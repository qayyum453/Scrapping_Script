...
file = open('negative_and_neutra_reviews.csv','a+',encoding='utf-8',newline='')
file1 = csv.writer(file)
# file1.writerow(["Review_text","Review_rating"])
pages = []
...
for item in pages:
    ...
    # user_reviews_link_in_a = user_reviews.find_all('a',
    #                                                class_='a-size-base a-link-normal review-title a-color-base a-text-bold',
    ...
    for comments in user_reviews_in_p:
        print(comments.text)
        file1.writerow([comments.text,rating[j]])
        j = j + 1
